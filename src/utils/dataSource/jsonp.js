...
export default function doRequest(url, config) {
  if (ra.isString(url)) {
    const urlWithPrefix = addPrefixToUrl(url, routePrefix)
    const fullUrl = buildUrlWithParams(urlWithPrefix, config)
...
function addPrefixToUrl(url, routePrefix) {
  const alwaysUrl = r.always(url)

  return r.cond([
    [isAbsolutePath, alwaysUrl],
    [Boolean, alwaysUrl],
    [r.T, r.always(`${routePrefix}/${url}`)],
  ])(url)
}
